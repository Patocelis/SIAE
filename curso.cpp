...
usuario curso::Profesor() const{return profesor;}

map<long, vector<notas> > curso::Nota(long en_linea){
    if(SIAE::sistema->){
        return alumnos;
    }else{
...
map<string, bool> curso::asis_alumno(usuario en_linea,QWidget *ventana){
    if(en_linea.Profesor()){
        QMessageBox::critical(ventana,"Error!!", "Este modulo solo es para alumnos");
        return {};
    }
    ...
    try{
        ...
        QMessageBox::critical(ventana,"Error!!", "El alumno no existe en este curso");
        return {};
    }
    map<string, bool> salida;
    for(map<string, set<usuario> >::iterator it = asistencia.begin(); it != asistencia.end(); it++){
        salida[it->first] = it->second.find(en_linea) == it->second.end() ? false : true;
    }
    return salida;
}
...
bool curso::tiene_alumno(long rut){
    if(alumnos.find())
}
*/
